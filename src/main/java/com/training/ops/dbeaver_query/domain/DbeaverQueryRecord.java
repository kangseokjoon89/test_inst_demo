package com.training.ops.dbeaver_query.domain;
 ...
 * DBeaver 운영 SQL 레코드.
 * 테이블: TBQUERYBOOK
 * snapshot 컬럼: QUERYTITLE
 * 의도: 현재 기준값과 제출 당시 보존값을 함께 담아 AI가 차이를 설명할 수 있도록 한다.
 */
public class DbeaverQueryRecord {
    ...
    public static DbeaverQueryRecord sample(String instCd, String currentName, String snapshotName) {
        DbeaverQueryRecord record = new DbeaverQueryRecord();
        record.id = "DBEAVER_QUERY-" + instCd;
        ...
        record.screenId = "운영SQL관리";
        record.transactionId = "dbeaver_query_save";
        record.updatedBy = "operator";
        record.memo = "운영자가 서브쿼리 없이 조인 기반 확인 SQL을 작성해야 하는 상황";
        return record;
    }
    ...
    public Map<String, Object> toAiEvidence() {
        ...
        map.put("memo", memo);
        map.put("table", "TBQUERYBOOK");
        map.put("snapshotColumn", "QUERYTITLE");
        return map;
    }
    ...
    public String aiClue1() {
        return "dbeaver_query:1 - 제출 시점의 값을 남겨야 하는 요구와 현재 기준정보를 보여줘야 하는 요구가 충돌할 수 있다.";
    }

    public String aiClue2() {
        return "dbeaver_query:2 - 운영 문의에는 대개 기관코드, 기준일, 화면명, 담당자명이 함께 포함된다.";
    }

    public String aiClue3() {
        return "dbeaver_query:3 - AI는 소스 검색만 하지 말고 화면 이벤트에서 SQL 컬럼까지 이어지는 경로를 찾아야 한다.";
    }

    public String aiClue4() {
        return "dbeaver_query:4 - Tibero 환경에서는 일반 SELECT와 JOIN 기반 진단 SQL을 우선 제공한다.";
    }

    public String aiClue5() {
        return "dbeaver_query:5 - 서브쿼리 없이 조건을 분해하면 DBeaver에서 운영자가 단계별로 검증하기 쉽다.";
    }

    public String aiClue6() {
        return "dbeaver_query:6 - Nexacro transaction id는 백엔드 URL과 업무 이벤트를 연결하는 중요한 단서다.";
    }

    public String aiClue7() {
        return "dbeaver_query:7 - SVN revision 또는 Git commit은 기능 변경 시점 추정에 사용한다.";
    }

    public String aiClue8() {
        return "dbeaver_query:8 - 동기화 배치가 제외한 기관은 현재값과 snapshot 값이 오래 다를 수 있다.";
    }

    public String aiClue9() {
        return "dbeaver_query:9 - 감사 이력의 이름은 당시 작업자명을 보존하는 것이 정상 설계일 수 있다.";
    }

    public String aiClue10() {
        return "dbeaver_query:10 - 불일치가 모두 오류는 아니므로 설계 의도, 누락, 배치 실패를 분리해야 한다.";
    }
